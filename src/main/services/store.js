...
export class StoreService {
  ...
  rowToAccount(row) {
    return {
      ...
      pool: row.pool,
      labels: fromJsonArray(row.labels),
      notes: row.notes,
  ...
  rowToTemplate(row) {
    return {
      ...
      type: row.type,
      name: row.name,
      schema: fromJson(row.schema) ?? {},
      version: row.version,
      isLocal: row.is_local === 1,
      updatedAt: row.updated_at
    }
  }
  rowToTaskTemplate(row) {
    return {
      id: row.id,
      name: row.name,
      version: row.version,
  ...
  rowToAirdropProject(row) {
    return {
      ...
      description: row.description,
      links: fromJsonArray(row.links),
      eligibilityCriteria: fromJsonArray(row.eligibility_criteria),
      tasks: fromJsonArray(row.tasks),
      earnings: fromJsonArray(row.earnings),
  ...
  getAccount(id) {
    const row = this.stmt('account.getById').get(id)
  ...
  getTemplate(id) {
    const row = this.stmt('template.getById').get(id)
    return row ? this.rowToTemplate(row) : null
  }
  listTemplates(page = 1, pageSize = 20, search) {
    if (search) {
      const countStmt = this.db.prepare(
  ...
  getTaskTemplate(id) {
    const row = this.stmt('taskTemplate.getById').get(id)
    return row ? this.rowToTaskTemplate(row) : null
  }
  listTaskTemplates(page = 1, pageSize = 20, search) {
    if (search) {
      const countStmt = this.db.prepare(
  ...
  getScheduledTask(id) {
    const row = this.stmt('scheduledTask.getById').get(id)
    return row ? this.rowToScheduledTask(row) : null
  }
  listScheduledTasks(page = 1, pageSize = 20, search) {
    if (search) {
      const countStmt = this.db.prepare(
  ...
  getAirdrop(id) {
    const row = this.stmt('airdrop.getById').get(id)
    return row ? this.rowToAirdropProject(row) : null
  }
  listAirdrops(page = 1, pageSize = 20, search) {
    if (search) {
      const countStmt = this.db.prepare(
  ...
  getCaptchaKey(id) {
    const row = this.stmt('captchaKey.getById').get(id)
    return row ? this.rowToCaptchaKey(row) : null
  }
  listCaptchaKeys(page = 1, pageSize = 20, search) {
    if (search) {
      const countStmt = this.db.prepare(
  ...
  getProxyProvider(id) {
    const row = this.stmt('proxyProvider.getById').get(id)
    return row ? this.rowToProxyProvider(row) : null
  }
  listProxyProviders(page = 1, pageSize = 20, search) {
    if (search) {
      const countStmt = this.db.prepare(
  ...
  getSetting(key) {
    const row = this.stmt('setting.get').get(key)
  ...
  getAllSettings() {
    const rows = this.stmt('setting.getAll').all()
    const result = {}
  ...
  getStats() {
    ...
    const accountTotal = this.stmt('account.count').get().cnt
    const accountPoolRows = this.stmt('account.countByPool').all()
    const accountPoolDistribution = {}
    for (const row of accountPoolRows) {
      accountPoolDistribution[row.pool] = row.cnt
    }
    ...
    for (const row of durDistRows) {
      taskDurationDistribution[row.bucket] = row.cnt
    }
    ...
    const templateRankingRows = this.db
      ...
      .all()
    const templateRanking = templateRankingRows.map((row) => {
      const tc = row.task_count
      const sc = row.success_count ?? 0
      const ec = row.error_count ?? 0
      return {
        templateName: row.template_name,
        taskCount: tc,
        successCount: sc,
  ...
  getAppInfo(version = '', dataDir = '') {
    ...
    const accountCount = this.stmt('account.count').get().cnt
  ...
  queryLogs(level, category, search, since, until, limit = 100) {
    const conditions = []
    const params = []
    if (level) {
    ...
    const countRow = this.db.prepare(`SELECT COUNT(*) as cnt FROM app_logs ${where}`).get(...params)
  ...
  getLogCategories() {
    const rows = this.db.prepare('SELECT DISTINCT category FROM app_logs ORDER BY category').all()
