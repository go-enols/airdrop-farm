...
import type { AirdropProject, AirdropStatus, AirdropProjectType, AirdropLink, AirdropTaskItem, Earning } from '../types'
import { Plus, Trash2, ExternalLink, Edit3, ChevronDown, ChevronUp, Link, DollarSign, ListChecks } from 'lucide-react'
...
const AIRDROP_TYPES: AirdropProjectType[] = ['testnet', 'mainnet', 'galxe', 'quest', 'social', 'other']
...
const Airdrops: React.FC = () => {
  ...
  const {
    ...
    search,
    refresh
  } = usePaginatedList<AirdropProject>(
    (p, ps, s) => airdropApi.list(p, ps, s),
    PAGE_SIZE
  )
  ...
  const [editForm, setEditForm] = useState<EditFormData>({
    name: '', chain: '', status: 'ongoing', projectType: 'testnet', description: '',
    links: [], tasks: [], earnings: [], tags: '', labels: ''
  })
  ...
  const addTask = () => {
    setEditForm(f => ({
      ...f,
      tasks: [...f.tasks, { id: crypto.randomUUID(), title: '', description: '', status: 'pending' as const, notes: '' }]
    }))
  }
  ...
  const addEarning = () => {
    setEditForm(f => ({
      ...f,
      earnings: [...f.earnings, { id: crypto.randomUUID(), token: '', amount: 0, date: new Date().toISOString().slice(0, 10), notes: '' }]
    }))
  }
  ...
  return (
    <div className="space-y-4">
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  <span className={`inline-block px-2 py-0.5 text-xs rounded-full font-medium ${STATUS_COLORS[item.status]}`}>
                    {t(STATUS_KEYS[item.status])}
                  </span>
                  <span className={`inline-block px-2 py-0.5 text-xs rounded-full font-medium ${TYPE_COLORS[item.projectType]}`}>
                ...
                {item.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {item.tags.map((tag, i) => (
                      <span key={i} className="inline-block px-2 py-0.5 text-xs bg-gray-100 text-text-muted rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>
                        ...
                        <div className="space-y-1">
                          {item.tasks.map((task, i) => (
                            <div key={i} className="text-xs text-gray-600">
                              ...
                              {task.description && <span className="text-gray-400 ml-1">— {task.description}</span>}
                            </div>
                          ))}
                        </div>
                      )}
                        ...
                        <div className="space-y-1">
                          {item.earnings.map((earning, i) => (
                            <div key={i} className="text-xs text-gray-600">
                              ...
                              {earning.valueUsd != null && <span className="text-gray-400 ml-1">(${earning.valueUsd})</span>}
                            </div>
                          ))}
                        </div>
                      )}
      ...
      <Modal open={showCreate} onClose={() => { setShowCreate(false); setCreateError(null) }} title={t('airdrops.createAirdrop')} scrollable>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.name')}</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.chain')}</label>
                <input
                  type="text"
                  value={form.chain}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.status')}</label>
                <select
                  value={form.status}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.projectType')}</label>
                <select
                  value={form.projectType}
                  onChange={(e) => setForm((f) => ({ ...f, projectType: e.target.value as AirdropProjectType }))}
                  className="w-full px-3 py-2 text-sm border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.description')}</label>
                <textarea
      ...
      <Modal open={!!editingItem} onClose={() => setEditingItem(null)} title={t('airdrops.editAirdrop')} maxWidth="max-w-lg" scrollable>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.name')}</label>
                <input
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditForm((f) => ({ ...f, status: e.target.value as AirdropStatus }))}
                  className="w-full px-3 py-2 text-sm border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                >
                  {AIRDROP_STATUSES.map((s) => (
                    <option key={s} value={s}>{t(STATUS_KEYS[s])}</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditForm((f) => ({ ...f, projectType: e.target.value as AirdropProjectType }))}
                  className="w-full px-3 py-2 text-sm border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                >
                  {AIRDROP_TYPES.map((tp) => (
                    <option key={tp} value={tp}>{t(TYPE_KEYS[tp])}</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.tags')}</label>
                <input
                  type="text"
                  value={editForm.tags}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('airdrops.labels')}</label>
                <input
                  type="text"
