...
import {
  Plus, Play, Pause, Square, Trash2, FileText, Edit3,
  CheckSquare as CheckSquareIcon, Square as SquareIcon, Trash, RefreshCw
} from 'lucide-react'
...
const Tasks: React.FC = () => {
  ...
  const [progressMap, setProgressMap] = useState<Record<string, { percent: number; message: string } | null>>({})
  ...
  const handleAction = async (action: 'start' | 'stop' | 'pause' | 'resume' | 'delete', id: string) => {
    try {
  ...
  const renderActionButtons = (task: Task) => {
    ...
    if (s === 'running') {
      return (
        <>
          <button
            onClick={(e) => { e.stopPropagation(); handleAction('pause', task.id) }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); handleAction('stop', task.id) }}
    ...
    if (s === 'paused') {
      return (
        <>
          <button
            onClick={(e) => { e.stopPropagation(); handleAction('resume', task.id) }}
    ...
    return (
      <>
        <button
          onClick={(e) => { e.stopPropagation(); handleAction('start', task.id) }}
        ...
        <button
          onClick={(e) => { e.stopPropagation(); handleAction('delete', task.id) }}
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-bg-tertiary">
                ...
                <th className="px-4 py-3 font-medium text-text-muted text-right">{t('tasks.actions')}</th>
              </tr>
            </thead>
            <tbody>
              {items.map((task) => (
                  ...
                  <td colSpan={6} className="p-0">
                    ...
                    >
                      <div className="px-4 py-3 w-10" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => toggleSelect(task.id)} className="text-text-muted hover:text-text-primary transition-colors">
                          {selectedIds.has(task.id) ? <CheckSquareIcon size={16} className="text-primary" /> : <SquareIcon size={16} />}
                        </button>
                      </div>
                      <div className="flex-1 grid grid-cols-[1fr_100px_160px_160px] items-center">
                        <div className="px-4 py-3 font-mono text-xs truncate text-text-primary">{task.scriptFolder}</div>
                        <div className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-status-${task.status}-bg text-status-${task.status}-text`}>
                            {task.status === 'running' && (
                        ...
                        <div className="px-4 py-3 text-text-secondary text-xs">{formatTime(task.startedAt)}</div>
                        <div className="px-4 py-3 text-text-secondary text-xs">{formatTime(task.endedAt)}</div>
                      </div>
                    ...
                    {task.status === 'running' && progressMap[task.id] && (
                      <div className="px-4 py-2 border-t border-border-light/50 bg-bg-tertiary/50">
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-text-muted shrink-0">{t('tasks.progress')}</span>
                          <div className="flex-1 h-2 bg-bg-card rounded-full overflow-hidden">
                            <div
                              className="h-full bg-primary rounded-full transition-all duration-500"
                              style={{ width: `${Math.min(100, Math.max(0, progressMap[task.id]!.percent))}%` }}
                            />
                          </div>
                      ...
                      <div className="border-t border-border-light bg-bg-tertiary/30">
                        <div className="flex items-center justify-between px-4 py-2 border-b border-border-light">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-text-muted">{t('tasks.logs')}</span>
                          </div>
                          <button
                            ...
                            <div className="space-y-0.5 font-mono text-xs">
                              {logs.map((log) => (
                                <div key={log.id} className="flex gap-3">
                                  ...
                                  <span className={`shrink-0 w-10 ${LOG_LEVEL_STYLES[log.level]}`}>[{log.level.toUpperCase()}]</span>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title={t('tasks.createTask')}>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">{t('tasks.selectTemplate', '从模板创建')}</label>
                <select
      ...
      <Modal open={showEdit && !!editTask} onClose={() => setShowEdit(false)} title={t('tasks.editTask')}>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">{t('tasks.scriptFolder')}</label>
                <input
