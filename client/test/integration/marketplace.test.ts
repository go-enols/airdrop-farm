...
async function assertGet(url: string, expectedStatus: number, checkBody: (body: any) => boolean, label: string) {
  try {
...
async function assertPost(url: string, expectedStatus: number, checkBody: (body: any) => boolean, label: string, headers?: Record<string, string>, bodyPayload?: any) {
  try {
    const res = await fetch(url, {
      method: 'POST',
...
async function assertPut(url: string, expectedStatus: number, checkBody: (body: any) => boolean, label: string, headers?: Record<string, string>, bodyPayload?: any) {
  try {
    const res = await fetch(url, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', ...headers },
...
async function assertDelete(url: string, expectedStatus: number, checkBody: (body: any) => boolean, label: string, headers?: Record<string, string>) {
  try {
    const res = await fetch(url, {
      method: 'DELETE',
      headers,
...
async function runTests() {
  ...
  // 1. Health check
  console.log('\n── Health Check ──')
  await assertGet(`${SERVER_URL}/api/health`, 200,
    (b) => b.status === 'ok',
  ...
  console.log('\n── Script Market: List ──')
  await assertGet(`${SERVER_URL}/api/scripts`, 200,
  ...
  await assertGet(`${SERVER_URL}/api/scripts/script-example-echo`, 200,
    (b) => b.data?.name === 'Echo 示例脚本' && b.data?.entryPoint === 'echo.sh',
    'GET /api/scripts/script-example-echo returns echo script'
  )
  await assertGet(`${SERVER_URL}/api/scripts/script-example-wallet-check`, 200,
  ...
  await assertGet(`${SERVER_URL}/api/scripts/non-existent`, 404,
  ...
  try {
    ...
    assert(dlRes.status === 200 && dlRes.headers.get('Content-Type')?.includes('zip'),
  ...
  await assertGet(`${SERVER_URL}/api/templates`, 200,
    (b) => b.data?.items?.length >= 2 && b.data?.total >= 2,
  ...
  await assertPost(`${SERVER_URL}/api/scripts`, 401,
  ...
  await assertPost(`${SERVER_URL}/api/templates`, 201,
    (b) => b.data?.name === 'Test Template' && b.data?.type === 'test',
    'POST /api/templates with valid auth creates template',
    authHeaders,
    { name: 'Test Template', type: 'test', version: '1.0.0', description: 'Integration test template', schema: { fields: [{ name: 'testField', type: 'string', label: 'Test', required: true }] } }
  )

  // 9. Update the created template
  console.log('\n── Template Market: Update ──')
  await assertPut(`${SERVER_URL}/api/templates/template-evm-wallet`, 200,
  ...
  try {
    ...
    const testTemplates = items.filter((t: any) => t.name === 'Test Template' || t.name === 'Duplicate Template')
    for (const tpl of testTemplates) {
      await assertDelete(`${SERVER_URL}/api/templates/${tpl.id}`, 200,
