...
router.post('/', upload.single('file'), (req: Request, res: Response) => {
  ...
  if (!name || !version) {
    ...
    res.status(400).json({ error: { message: 'name and version are required', code: 'VALIDATION_ERROR' } })
    return
  }
  ...
  try {
    ...
    execSync(`unzip -o "${req.file.path}" -d "${tmpDir}"`, { timeout: 30000 })
...
router.put('/:id', upload.single('file'), (req: Request, res: Response) => {
  ...
  const { name, version, description, entryPoint, tags, changelog, schema } = req.body
  const now = new Date().toISOString()
  ...
  if (req.file) {
    checksum = createHash('sha256').update(require('fs').readFileSync(req.file.path)).digest('hex')
