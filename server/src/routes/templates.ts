...
router.get('/', (req: AuthenticatedRequest, res: Response) => {
  ...
  const rows = (showAll ? stmts.templateGetAllAdmin : stmts.templateGetAll).all() as Record<string, unknown>[]
...
router.post('/', requireRole('admin', 'developer'), (req: AuthenticatedRequest, res: Response) => {
...
router.put('/:id', requireRole('admin', 'developer'), (req: AuthenticatedRequest, res: Response) => {
...
router.patch('/:id', requireRole('admin', 'developer'), (req: AuthenticatedRequest, res: Response) => {
  ...
  if (name || type || version || description !== undefined || schema !== undefined) {
    const now = new Date().toISOString()
    const schemaJson = typeof schema === 'string' ? schema : JSON.stringify(schema ?? JSON.parse((existing.schema as string) || '{}'))
    stmts.templateUpdate.run(
      name ?? existing.name,
      type ?? existing.type,
      version ?? existing.version,
