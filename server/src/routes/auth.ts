...
router.post('/register', authMiddleware, requireRole('admin'), (req: AuthenticatedRequest, res: Response) => {
  ...
  if (password.length < 4) {
    res.status(400).json({
      error: { message: 'password must be at least 4 characters', code: 'VALIDATION_ERROR' }
    })
    return
  }
  ...
  if (existing) {
    res.status(409).json({ error: { message: 'Username already exists', code: 'CONFLICT' } })
    return
  }
